...
// Helper function to generate deep smart context-aware responses in Bengali/English
...
app.post("/api/chat", async (req: any, res: any) => {
  try {
    ...
    if (!apiKey) {
      console.warn("GEMINI_API_KEY is not defined. Falling back to fallback response.");
    ...
    const systemInstruction = `
      You are MSKE Shipping AI Support (এমএসকেএ শিপপে ট্রায়াল এআই সাপোর্ট অ্যাসিস্ট্যান্ট).
      ...
      - 2000/5000 Taka package corresponding TrxIDs: DE2000LK8M, DE20QW5RX2, DE200AZ7PN, DE20M4YT9K, DE2000VF6C
    ...
    console.warn("Gemini API call warning in serverless function:", error.message);
    ...
    if (matchedId) {
      ...
      try {
        ...
        for (const ext of extensions) {
          try {
            fetchedBuffer = await tryFetch(`https://images.pasteboard.co/${matchedId}.${ext}`);
            break;
          } catch {
            continue;
          }
